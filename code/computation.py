...
class LLFlow_wrapper(nn.Module):
    def __init__(self, model):
        super(LLFlow_wrapper, self).__init__()
        self.model = model
        self.netG = model.netG
        self.opt = model.opt
    def forward(self, x, his):
        """
        x: tensor with the shape B*3*H*W
        his: the histogram equalized patch (first do histogram equalization, then crop the same patches)
        """
        lr_t = x # (B, 3, 160, 160)
        ...
        hr = self.model.get_sr(lq=lr_t.cuda(), heat=None)
        z = self.model.get_z(0, None, batch_size=lr_t.shape[0], lr_shape=lr_t.shape)
        hr, logdet = self.netG(lr=lr_t, z=z, eps_std=0, reverse=True, epses=None)
        # B*3*H*W
        return hr
    
def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--opt", default="/home/yufei/shulin/LLFlow_v1/code/confs/IR_smallNet.yml")
    args = parser.parse_args()
    conf_path = args.opt
    conf = conf_path.split('/')[-1].replace('.yml', '')
    model, opt = load_model(conf_path)
    model.netG = model.netG.cuda()
    model_llflow = LLFlow_wrapper(model)
    
    
    lr_dir = opt['dataroot_LR'] # '/home/yufei/shulin/LLFlow_v1/IR-RGB-Dataset-resize/eval/low'
