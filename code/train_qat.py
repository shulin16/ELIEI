...
from code.data.dataset import LoL_Dataset, LoL_Dataset_v2, IR_Dataset
...
def main():
    ...
    parser.add_argument('--opt', type=str, help='Path to option YMAL file.',
                            default='/home/yufei/shulin/ELIEI/code/confs/IR_smallNet.yml' if sys.platform != 'win32' else './confs/LOL_smallNet.yml')
    ...
    example_inputs = torch.randn(4,6,160,160)  # LQ
    model_to_quantize.netG.eval()
    qconfig_mapping = QConfigMapping().set_global(torch.quantization.get_default_qat_qconfig('fbgemm'))
    model_to_quantize.netG.train()
    # prepare
    model = quantize_fx.prepare_qat_fx(model_to_quantize, qconfig_mapping, example_inputs)
