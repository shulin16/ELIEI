...
def histc_diff(x, bins=None, min=0.0, max=1.0):
    if len(x.shape) == 4:
...
def get_hist(x):
    ...
    for i in range(x.shape[0]):
        for c in range(x.shape[1]):
            hist_x = histc_diff(x[i][c], bins=255, min=0, max=1)
            # Normalize
            hist_x /= hist_x.sum()
            hist_x = hist_x.unsqueeze(0) # [1,H,W]
            if c ==0:
                hist_c = hist_x
            else:
                hist_c = torch.cat([hist_c, hist_x], dim=0)
        # hist_c: [c, H, W]
        hist_c.unsqueeze(0)
        if i == 0:
            hist = hist_c
        else:
            hist = torch.cat([hist, hist_c], dim=0)
    return hist